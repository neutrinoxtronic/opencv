...
public:
    ...
    virtual inline vtype parallel(const vtype& src, const vtype& thresh, const vtype& maxval)
    {
        (void)src;
        (void)thresh;
        (void)maxval;
    ...
    virtual inline type nonparallel(const type& src, const type& thresh, const type& maxval)
    {
        (void)src;
        (void)thresh;
        (void)maxval;
...
class opThreshBinary : public operators_threshold_t<type, vtype> {
    inline vtype parallel(const vtype& src, const vtype& thresh, const vtype& maxval) override
    {
        return (vtype)__nds__bpick((long)maxval, (long)0, (long)(src > thresh));
    }
    inline type nonparallel(const type& src, const type& thresh, const type& maxval) override
    {
...
static void ndsrvp_threshold_op(const type* src_data, size_t src_step,
    ...
    type thresh, type maxval, int thtype)
{
    ...
    for (i = 0; i < height; i++, src_data += src_step, dst_data += dst_step) {
        for (j = 0; j <= width - nlane; j += nlane) {
            ...
            *(vtype*)(dst_data + j) = op->parallel(vs, vthresh, vmaxval);
        }
        for (; j < width; j++) {
            dst_data[j] = op->nonparallel(src_data[j], thresh, maxval);
        }
    }
    ...
    double thresh, double maxValue, int thresholdType)
{
    ...
    if (depth == CV_8U) {
        ndsrvp_threshold_op<uchar, uint8x8_t, 8>((uchar*)src_data, src_step, (uchar*)dst_data, dst_step, width, height, cn, (uchar)thresh, (uchar)maxValue, thresholdType);
        return CV_HAL_ERROR_OK;
    } else if (depth == CV_16S) {
        ndsrvp_threshold_op<short, int16x4_t, 4>((short*)src_data, src_step, (short*)dst_data, dst_step, width, height, cn, (short)thresh, (short)maxValue, thresholdType);
        return CV_HAL_ERROR_OK;
    } else if (depth == CV_16U) {
        ndsrvp_threshold_op<ushort, uint16x4_t, 4>((ushort*)src_data, src_step, (ushort*)dst_data, dst_step, width, height, cn, (ushort)thresh, (ushort)maxValue, thresholdType);
        return CV_HAL_ERROR_OK;
    } else {
        return CV_HAL_ERROR_NOT_IMPLEMENTED;
    }
