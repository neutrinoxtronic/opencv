...
struct ResizeOnnx
{
    ...
    void run()
    {
        ...
        Mat S, R = iR, nS, nR;
        cv::copyMakeBorder(iS, S, 1, 2, 3, 4, cv::BORDER_CONSTANT);
        S = S(Rect(3, 1, szsrc.width, szsrc.height));
        ...
        for (int cn = 1; cn <= 6; ++cn)
        {
            ...
            for (int depth = CV_8U; depth <= CV_64F; ++depth)
            {
                ...
                // cvRound(4.5) = 4, but when doing resize with int, we may get 5
                    ...
                    EXPECT_LE(cv::norm(ref, dst, NORM_INF), 1.0) << errinfo;
                else 
                    EXPECT_LE(cv::norm(ref, dst, NORM_INF), 1e-4) << errinfo;
            }
        }
    }
};
